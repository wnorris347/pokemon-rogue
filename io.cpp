...
void io_choose_starter(){
  game_pokemon starter1, starter2, starter3;
  char input;
  ...
  starter1 = generate_pokemon();
  starter2 = generate_pokemon();
  starter3 = generate_pokemon();

  mvprintw(3, 20, "Choose one of the starters using the");
  mvprintw(4, 20, "corresponding number key");

  mvprintw(6, 20, "1. %s", starter1.name);
  mvprintw(7, 20, "2. %s", starter2.name);
  mvprintw(8, 20, "3. %s", starter3.name);

  correct_key_pressed = 0;
  while(!correct_key_pressed){
    switch(input = getch()){
      case '1':
        world.pc.team.push_back(starter1);
        correct_key_pressed = 1;
        break;
      case '2':
        world.pc.team.push_back(starter2);
        correct_key_pressed = 1;
        break;
      case '3':
        world.pc.team.push_back(starter3);
        correct_key_pressed = 1;
        break;
      default:
        mvprintw(11, 20, "Unmapped input: %c", input);
        break;
    }
  }
...
void io_pokemart()
{
  mvprintw(0, 0, "Welcome to the Pokemart.  How can I help you?  --more--");
  getch();
  move(0,0);
  clrtoeol();
  mvprintw(0, 0, "Let's get you restocked. (All items restored to 10)");
  world.pc.num_potions = 10;
  world.pc.num_revives = 10;
  world.pc.num_p_balls = 10;
  refresh();
  getch();
}
...
void io_pokemon_center()
{
  ...
  mvprintw(0, 0, "Welcome to the Pokemon Center.  How can Nurse Joy assist you?  --more--");
  getch();
  move(0,0);
  clrtoeol();
  mvprintw(0, 0, "Thanks for waiting, we've restored all your Pokemon to full health");
  for(i = 0; i < (int)world.pc.team.size(); i++){
    world.pc.team.at(i).battleStat[stat_hp] = world.pc.team.at(i).currentStat[stat_hp];
    for(j = 0; j < (int)world.pc.team.at(i).moves.size(); j++){
      world.pc.team.at(i).moves.at(j).current_pp = world.pc.team.at(i).moves.at(j).max_pp;
    }
  }
  refresh();
  getch();
  move(0,0);
  clrtoeol();
  mvprintw(0, 0, "We hope to see you again!");
  refresh();
  getch();
}
...
void io_wild_battle(game_pokemon encounter){
  ...
  io_display();
  move(0,0);
  printw("A wild %s appeared!", encounter.name);
  refresh();
  getch();

  battle_outcome = wild_battle_loop(&encounter);

  if(battle_outcome == 1){
    move(0,0);
    clrtoeol();
    mvprintw(0, 0, "You are out of usable Pokemon...");
    refresh();
    getch();
    move(0,0);
    clrtoeol();
    ...
    getch();
    world.quit = 1;
  }else if(!battle_outcome){
    return;
  }else if(battle_outcome == 2){
    if((int)world.pc.team.size() < 6){
      move(0,0);
      clrtoeol();
      mvprintw(0, 0, "Threw a Poke ball!");
      getch();
      move(0,0);
      clrtoeol();
      mvprintw(0, 0, "Gotcha! %s was caught!", encounter.name);
      getch();
      world.pc.team.push_back(encounter);
    }else{
      move(0,0);
      clrtoeol();
      mvprintw(0, 0, "Oh no! The Pokemon broke free!");
      getch();
      move(0,0);
      clrtoeol();
      mvprintw(0, 0, "The wild %s fled...", encounter.name);
    }
  }

  if(!world.quit){
    for(i = 0; i < (int)world.pc.team.size(); i++){
      if(world.pc.team.at(i).leveled_last_battle && world.pc.team.at(i).level >= world.pc.team.at(i).evolves_at_level){
        evolve(&world.pc.team.at(i));
      }
    }
  }
}
...
void io_trainer_battle(character *aggressor, character *defender)
{
  ...
  game_pokemon npc_active_pokemon, player_active_pokemon;
  std::vector<game_pokemon> npc_pokemon_remaining, player_pokemon_remaining;
  char trainer_name[10], battle_message[80], pc_first[80];

  for(i = 0; i < (int)world.pc.team.size(); i++){
    if(world.pc.team.at(i).battleStat[stat_hp]){
      player_pokemon_remaining.push_back(world.pc.team.at(i));
    }
  }

  for(i = 0; i < (int)n->team.size(); i++){
    npc_pokemon_remaining.push_back(n->team.at(i));
  }

  player_active_pokemon = player_pokemon_remaining.at(0);
  npc_active_pokemon = n->team.at(0);

  switch(n->ctype){
    case char_rival:
    strcpy(trainer_name, "Rival");
    break;
    case char_hiker:
    strcpy(trainer_name, "Hiker");
    break;
    case char_swimmer:
    strcpy(trainer_name, "Swimmer");
    break;
    case char_other:
    strcpy(trainer_name, "Trainer");
    break;
    default:
    break;
  }
  ...
  sprintf(battle_message, "%s challenged you!", trainer_name);
  io_queue_message(battle_message);

  sprintf(battle_message, "%s sent out %s!", trainer_name, npc_active_pokemon.name);
  sprintf(pc_first, "Go! %s!", player_active_pokemon.name);

  io_queue_message(battle_message);
  io_queue_message(pc_first);
  ...
  battle_outcome = trainer_battle_loop(npc_pokemon_remaining, trainer_name);
  
  if(!battle_outcome){
    move(0,0);
    clrtoeol();
    mvprintw(0, 0, "You defeated %s!", trainer_name);
    getch();
    n->defeated = 1;
  ...
  if(!world.quit){
    for(i = 0; i < (int)world.pc.team.size(); i++){
      ...
      world.pc.team.at(i).leveled_last_battle = 0;
    }
  }
}
