...
pokemon_evolution_db evolutions[460];
...
void db_parse(bool print)
{
  ...
  prefix = (char *) realloc(prefix, prefix_len + strlen("pokemon_evolutions.csv") + 1);
  strcpy(prefix + prefix_len, "pokemon_evolution.csv");
  ...
  index = 1;
  while(getline(stream, line2) && index < 460){
    start = 0;
    evolutions[index].id = stoi(line2.substr(0, line2.find(",")));
    start = line2.find(",") + 1;
    evolutions[index].evolved_species = stoi(line2.substr(start, line2.length()).substr(0, line2.substr(start, line2.length()).find(",")));
    start += line2.substr(start, line2.length()).find(",") + 1;
    evolutions[index].evolution_trigger = ((line2.substr(start, line2.length()).substr(0, line2.substr(start, line2.length()).find(",")).length()) == 0) ? INT_MAX : (stoi(line2.substr(start, line2.length()).substr(0, line2.find(","))));
    start += line2.substr(start, line2.length()).find(",") + 1;
    evolutions[index].trigger_item = ((line2.substr(start, line2.length()).substr(0, line2.substr(start, line2.length()).find(",")).length()) == 0) ? INT_MAX : (stoi(line2.substr(start, line2.length()).substr(0, line2.find(","))));
    start += line2.substr(start, line2.length()).find(",") + 1;
    evolutions[index].minimum_level = ((line2.substr(start, line2.length()).substr(0, line2.substr(start, line2.length()).find(",")).length()) == 0) ? INT_MAX : (stoi(line2.substr(start, line2.length()).substr(0, line2.find(",") + 1)));
    start += line2.substr(start, line2.length()).find(",") + 1;
    evolutions[index].gender_id = ((line2.substr(start, line2.length()).substr(0, line2.substr(start, line2.length()).find(",")).length()) == 0) ? INT_MAX : (stoi(line2.substr(start, line2.length()).substr(0, line2.find(","))));
    index++;
  }
